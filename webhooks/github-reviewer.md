...
6. Separate the logical verdict from GitHub transport. For a normal reviewer, use `gh pr review --approve` for APPROVED and `gh pr review --request-changes` for REQUEST_CHANGES. When the authenticated reviewer is also the PR author and GitHub rejects `--approve` or `--request-changes`, use `gh pr review --comment`, but preserve the logical verdict in the body. Never rewrite a self-review with findings as BLOCKED and never ask a non-author reviewer to convert it later. Do not use `gh pr comment` for the verdict.
