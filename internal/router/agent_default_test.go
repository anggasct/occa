...
func TestAgentNoSessionListedSenderSetsChannelDefault(t *testing.T) {
...
func TestAgentNoSessionDefaultClearsAtCallerScope(t *testing.T) {
	...
	st2 := r.store.(*fakeStore)
	st2.channelRepo.channels["telegram:chat1"] = &store.Channel{
		ChannelID: "chat1", Platform: "telegram", ListenMode: "mention", Agent: "reviewer",
	}
	reply2 := &fakeReplyCtx{}
	if err := r.Route(context.Background(), msg("/agent default", reply2)); err != nil {
		t.Fatalf("Route clear: %v", err)
	}
	if len(reply2.sends) == 0 || !strings.Contains(reply2.sends[0], "Channel agent cleared") {
		t.Fatalf("expected channel clear confirmation, got %v", reply2.sends)
	}
	if ch := st2.channelRepo.channels["telegram:chat1"]; ch.Agent != "" {
		t.Fatalf("channel agent = %q, want empty", ch.Agent)
	}
}
...
func TestAgentNoSessionGuidanceText(t *testing.T) {
	...
	if !strings.Contains(reply.sends[0], "channel default") {
		t.Fatalf("expected channel default line, got %q", reply.sends[0])
	}
	if strings.Contains(reply.sends[0], "Send a message first") {
...
func TestAgentNewSessionPrefersPersonalOverride(t *testing.T) {
	r, client, reply := newTestRouter()
	client.agents = agentTestAgents()
	st := r.store.(*fakeStore)
	st.channelRepo.channels["telegram:chat1"] = &store.Channel{
	...
	o := &store.UserOverride{ChannelID: "chat1", Platform: "telegram", UserID: "user1", Agent: "planner"}
