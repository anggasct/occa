...
func TestInitBotWithRetrySucceedsAfterTransientFailures(t *testing.T) {
	...
	bot, err := initBotWithRetry("faketoken", ts.URL+"/bot%s/%s", &http.Client{Timeout: 5 * time.Second}, time.Millisecond, 3)
	if err != nil {
...
func TestInitBotWithRetryFailsAfterAllAttempts(t *testing.T) {
	...
	if got := atomic.LoadInt32(requests); got != 3 {
		t.Fatalf("requests = %d, want %d (bounded, no infinite retry)", got, 3)
	}
}
