	...
	for w := 0; w < writers; w++ {
		...
		go func(w int) {
			...
			for i := 0; i < perWriter; i++ {
				...
				errs <- err
				err = s.OverrideRepo().UpsertAgent(ctx, platform, channelID, userID, "reviewer")
				errs <- err
			}
		}(w)
	}
...
func TestQueriesUseIndexes(t *testing.T) {
	...
	statements := []struct {
		...
		{"override get", `SELECT id, channel_id, platform, user_id, model, agent, created_at, updated_at FROM user_override WHERE platform = ? AND channel_id = ? AND user_id = ?`, []any{"telegram", "chat1", "user1"}},
		{"override list", `SELECT id, channel_id, platform, user_id, model, agent, created_at, updated_at FROM user_override WHERE platform = ? AND channel_id = ? ORDER BY created_at`, []any{"telegram", "chat1"}},
