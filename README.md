...
```sh
curl -fsSL https://github.com/anggasct/occa/releases/latest/download/install.sh | bash
```

The installer detects your OS/arch, downloads the matching release binary, and
installs it to `$OCCA_INSTALL_DIR` → `$XDG_BIN_DIR` → `$HOME/bin` (in that
order). If `opencode` is not already on `PATH`, it runs OpenCode's official
installer.

To install a specific release instead of the latest:

```sh
export OCCA_VERSION=v0.1.0
curl -fsSL https://github.com/anggasct/occa/releases/latest/download/install.sh | bash
```
